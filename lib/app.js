...
var httpdataroot = __dirname + '/../'

// this is necessary, because different operating systems put node modules folder in different folders. How lucky we are.
var static_roots = [httpdataroot, __dirname + '/../node_modules', __dirname + '/../../', __dirname + '/../../../',];
...
try {
    ...
    var path = httpdataroot +'data/statlist.json';
    ...
    // fs.accessSync(httpdataroot +'data/statlist.json', fs.F_OK)     // this trows an no such method error on linux for some reson    
...
if (!hasFilesAlready){
    console.log('Seems like '+httpdataroot +'data/statlist.json'+' does not exists');
}
...
function startWebServer(){    
    ...
    var fileServers = static_roots.map(function(path){ return new static.Server(path);}) 
    ...
    require('http').createServer(function (req, res) {
        ...
        if (req.method == 'POST') {
            ...
            // static file serving on get. On 404 try other paths until there is other
            req.addListener('end', function () {                                
                var i = 0;
                fileServers[0].serve(req, res, serve)                                
                function serve(err){              
                    if (err && err.status == 404){                        
                        if (i < static_roots.length){
                            // console.log('|', 404,req.url, fileServers[i]?fileServers[i].root:'')
                            i++;
                            fileServers[i].serve(req, res, serve)
                        }
                        else{
                            // console.log('X - not found anywhere')
                        }
                        
                    }
                    else{
                        // console.log('O', 200, req.url, fileServers[i]?fileServers[i].root:'')
                    }                    
                }
...
function startConversion(){
    ...
    mkdirp(httpdataroot +'data/');

    var statlist = parseFbFiles(fbhistoryfiles)

    // export stat file, to be accessible via static
    jsonfile.writeFileSync(httpdataroot + 'data/'+utils.statFileListName, statlist);
...
function parseOneFile(inputFileName) {
    ...
    export_sqlite.exportMessages(messageData, httpdataroot+'data/stat.' + userName + '.sqlite');
    ...
    if (process.argv.indexOf('--no-messages')==-1)
    {
        ...
        fs.writeFileSync(httpdataroot +'data/messages.' + userName + '.json', json);
    }            
    return userName;
}
