...
var help = fs.readFileSync('./assets/help.md', "utf8");
...
try {
    ...
    fs.accessSync('./data/statlist.json', fs.F_OK)    
...
function startConversion(){
    ...
    jsonfile.writeFileSync('data/'+utils.statFileListName, statlist);
...
function parseOneFile(inputFileName) {
    ...
    export_sqlite.exportMessages(messageData, 'data/stat.' + userName + '.sqlite');
    ...
    if (process.argv.indexOf('--no-messages')==-1)
    {
        ...
        fs.writeFileSync('data/messages.' + userName + '.json', json);
    }            
    return userName;
}
