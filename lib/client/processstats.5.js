...
(function ($, SVG, moment) {
        ...
        for(var i = 0; i<d.length; i++){
            var a = $('<a>',{                    
                    'data-filename': d[i].fileName
                                ...
                                loadStat1(this.dataset.filename)
                        })
            $('#statlist').append($('<li>').append(a));
            
        }
        loadStat1(d[0].fileName)
    })    
     
     
    function loadStat1(filename){
        $.getJSON('data/stats.'+filename+'.json', function (d) {   
            ...
            drawMainUserLine(timeline, c, d)
            
            drawRegularLines(timeline, c, d)
            
            drawTrespassers(timeline, c, d)
            
            createMouseOvers(d.usermap);
    ...
    function drawExitingLine(timeline, ld){
        var path = {
            M: [ld.slotEndX, ld.y2], 
            Q: [    ld.slotEndX+(c.monthsWidth/3),ld.y2,
                    (ld.slotEndX+(c.monthsWidth)/2), (ld.y2+(c.monthsWidth)/2)],
            T: [(ld.slotEndX+(c.monthsWidth)/2), (ld.y2+(c.monthsWidth)/2)]};
    ...
    function drawEnteringLine(timeline, ld){
        ...
        var slotStartXstamp = ld.ientry.timeslot*c.monthsWidth;
        var ystamp = ld.ientry.order * c.lineHeight + c.offsetYp;
        createUserTextNode(timeline, ld.user, slotStartXstamp, ystamp, 'normal', ld);
    ...
    function getUser(key){
        ...
        if (!data.usermap[key]) return { name: 'INVALID USER', color: 'red', cnt: 0};        
